...
def enc(tag: str):
    """Безопасное кодирование тега (# -> %23)"""
...
async def cmd_start(message: Message):
    await message.answer(
        ...
        "⚠️ *ВАЖНО:* Перед началом напиши `/getip`, чтобы узнать IP бота и добавить его в белый список на developer.brawlstars.com\n\n"
        "📌 *Команды:*\n"
...
async def cmd_getip(message: Message):
    ...
    await message.answer(
        ...
        f"4. Нажми *Save*\n"
        f"5. Подожди 10 секунд и добавляй клубы!",
...
async def cmd_addclub(message: Message):
    ...
    if len(args) < 2: return await message.reply("❗ Укажи тег: `/addclub #ТЕГ`", parse_mode="Markdown")
    ...
    if status == 403:
        return await message.reply("🚫 *Ошибка 403 (Доступ запрещен)*\nIP-адрес бота не добавлен в белый список!\nНапиши `/getip`, чтобы узнать IP.", parse_mode="Markdown")
    if not club:
        return await message.reply(f"❌ Клуб не найден (Ошибка {status}). Проверь тег.")
    ...
    if tag in data[chat_id]:
        return await message.reply(f"ℹ️ Ты уже отслеживаешь *{club['name']}*.", parse_mode="Markdown")
        
    # Получаем участников для первичного сохранения
...
async def cmd_myclubs(message: Message):
    ...
    if chat_id not in data or not data[chat_id]:
        return await message.reply("📭 Нет отслеживаемых клубов.")
    text = "📋 *Твои клубы:*\n\n" + "\n".join([f"🔹 *{info['name']}* (`{tag}`)" for tag, info in data[chat_id].items()])
    await message.reply(text, parse_mode="Markdown")

@dp.message(Command("removeclub"))
async def cmd_removeclub(message: Message):
    args = message.text.split()
    if len(args) < 2: return await message.reply("❗ Укажи тег: `/removeclub #ТЕГ`", parse_mode="Markdown")
    tag = args[1].upper()
    if not tag.startswith("#"): tag = "#" + tag
    ...
    if chat_id in data and tag in data[chat_id]:
        ...
        save_data(data)
        await message.reply(f"✅ Клуб `{tag}` удален.", parse_mode="Markdown")
    else:
        await message.reply("❌ Ты не отслеживаешь этот клуб.")

@dp.message(Command("clubinfo"))
async def cmd_clubinfo(message: Message):
    args = message.text.split()
    if len(args) < 2: return await message.reply("❗ Укажи тег: `/clubinfo #ТЕГ`", parse_mode="Markdown")
    tag = args[1].upper()
    if not tag.startswith("#"): tag = "#" + tag
    
    club, status = await bs_api(f"/clubs/{enc(tag)}")
    if status == 403: return await message.reply("🚫 *Ошибка 403*. Напиши `/getip`.", parse_mode="Markdown")
    if not club: return await message.reply("❌ Клуб не найден.")
...
async def monitor_loop():
    ...
    while True:
        data = load_data()
        ip_warning_sent = False # Флаг, чтобы не спамить предупреждениями об IP
        
        for chat_id, clubs in data.items():
            for tag, old_info in list(clubs.items()):
                club, status = await bs_api(f"/clubs/{enc(tag)}")
                
                if status == 403 and not ip_warning_sent:
                    try:
                        await bot.send_message(chat_id, "🚨 *ВНИМАНИЕ!* Мой IP-адрес изменился и API блокирует доступ (Ошибка 403).\nНапиши `/getip`, чтобы узнать новый IP и обновить его на developer.brawlstars.com", parse_mode="Markdown")
                        ip_warning_sent = True
                    except: pass
                    continue # Пропускаем проверку, пока не починят IP
